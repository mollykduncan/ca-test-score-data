...
Similar to the previous post about extracting New York state's education data, this post is about programmatically extracting school-level standardized test results from a state's DOE website--in this case, the state of California. Luckily, California reports their data all the way back to 2003 in .csv files, which means we can write a little more elegant script to extract *all* of it, instead of the piecemeal approach we had to take with New York's data. 
...
We'll start with pulling the more recent results and work backwards.
...
```
This dataset isn't quite in its final form yet. For example, there are some variables that are currently character vectors that I'll want to be numeric (more on that later). But it's a good enough form to join it to the older datasets, and then we can do some more data cleaning on the single, joined dataset. So at this point, I'm going to write this dataset to disc as a couple of CSVs (because writing it as a single csv makes it just over the threshold of file sizes github will allow), and we'll call it a day.
```{r write-data}
